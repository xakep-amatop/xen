                              ...
                              bool valid)
{
    ...
    alloc = atomic_inc_return(&vpe->vmapp_count) == 1;
    ...
    /*
     * Xen can remap a VPE after unmapping it to sample VLPI state, so the VPT
     * is not guaranteed to be empty. Never advertise PTZ.
     */
    ptz = false;
