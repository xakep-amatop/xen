...
static int its_handle_mapti(struct virt_its *its, uint64_t *cmdptr)
{
    ...
    vgic_init_pending_irq(pirq, intid, false);
 ...
 out_remove_vlpi_host_entry:
    if ( pirq_is_tied_to_hw(pirq) )
    {
        int unmap_ret = gicv4_its_vlpi_unmap(pirq);

        /*
         * If we fail to tear down the VLPI mapping, keep the remaining
         * software state intact instead of tearing it down underneath a
         * still-live hardware mapping.
         */
        if ( unmap_ret )
            return unmap_ret;
    }
