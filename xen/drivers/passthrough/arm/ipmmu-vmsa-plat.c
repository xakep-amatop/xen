 ...
 * (A3VP, A3VP, A3IR) is identically configured across all SoCs (H3, M3
 * and M3N). This allow us not to introduce support for each SoC
 * separately.
 */
 ...
 * (IPMMU-VPx, IPMMU-VCx, IPMMU-IR belong to A3xx power domains) and
 * as the result they are in power-off state during booting, therefore
 * they must be explicitly powered on before initializing.
 */
...
bool ipmmu_is_mmu_tlb_disable_needed(struct dt_device_node *np)
{
	...
	/* W/A is actual for M3N only */
	if ( !dt_device_is_compatible(np, "renesas,ipmmu-r8a77965") )
		return false;
